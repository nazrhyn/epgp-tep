...
EPGP = LibStub("AceAddon-3.0"):NewAddon("EPGP-TEP", "AceEvent-3.0", "AceConsole-3.0")
local EPGP = EPGP
...
local version = GetAddOnMetadata('EPGP-TEP', 'Version')
...
function EPGP:DecodeNote(note)
    if note then
        if note == "" then
            ...
            local ep, gp, tep = string.match(note, "^(%d+),(%d+),(%d+)$")
            if ep then
                return tonumber(ep), tonumber(gp), tonumber(tep)
            end
        end
    end
end

local function EncodeNote(ep, gp, tep)
...
local function AddEPGP(name, ep, gp, suppressTotal)
    local current_ep = ep_data[name]
    local current_tep = tep_data[name]
    local current_gp = gp_data[name]
    assert(current_ep ~= nil and current_gp ~= nil and current_tep ~= nil,
        string.format("%s is not a main!", tostring(name)))

    local result_ep = current_ep + ep
    local result_gp = current_gp + gp
...
local comparators = {
    ...
    EP = function(a, b)
        local a_ep = EPGP:GetEPGP(a)
        local b_ep = EPGP:GetEPGP(b)

        return a_ep > b_ep
    end,
    GP = function(a, b)
        local _, a_gp = EPGP:GetEPGP(a)
        local _, b_gp = EPGP:GetEPGP(b)
    ...
    TEP = function(a, b)
        local _, _, a_tep = EPGP:GetEPGP(a)
        local _, _, b_tep = EPGP:GetEPGP(b)
...
local function ParseGuildNote(callback, name, note)
    ...
    local ep, gp, tep = EPGP:DecodeNote(note)
    if ep then
        ep_data[name] = ep
        gp_data[name] = gp
        tep_data[name] = tep
...
function EPGP:StandingsSort(order)
    ...
    assert(comparators[order], string.format("%s is an unknown sort order.", order))
...
function EPGP:DecayEPGP()
    ...
    for name, _ in pairs(ep_data) do
        -- Total EP is not decayed
        local ep, gp, _, main = self:GetEPGP(name)
        assert(main == nil, "Corrupt alt data!")
        local decay_ep = math.ceil(ep * decay)
        local decay_gp = math.ceil(gp * decay)
        decay_ep, decay_gp = AddEPGP(name, -decay_ep, -decay_gp, true)
        if decay_ep ~= 0 then
...
function EPGP:GetEPGP(name)
    ...
    if ep_data[name] then
        return ep_data[name], gp_data[name] + self.db.profile.base_gp, tep_data[name], main
    end
end
...
function EPGP:SetGlobalConfiguration(decay_p, extras_p, base_gp, min_ep, outsiders)
    ...
    local epgp_stanza = string.format("-EPGP-\n@DECAY_P:%d\n@EXTRAS_P:%s\n@MIN_EP:%d\n@BASE_GP:%d\n@OUTSIDERS:%d\n-EPGP-",
...
function EPGP:ReportErrors(outputFunc)
    for name, note in pairs(ignored) do
        outputFunc(L["Invalid officer note [%s] for %s (ignored)"]:format(note, name))
    end
end
...
function EPGP:OnInitialize()
    ...
    self.db = LibStub("AceDB-3.0"):New("EPGP_TEP_DB")
